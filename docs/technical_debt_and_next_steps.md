...
- **Done (2026-06-24):** ~~Fix `TaskLocalization` shared-pointer ownership bugs
  ...
  `DeviceManager` control block. See `docs/later_todo_list.md` item 22.1.
...
- **Done (2026-06-24):** ~~Snapshot `MatchGroup` data before worker-thread
  ...
  architecture contract suite (38 passed). See `docs/later_todo_list.md` item 22.6.
...
- **Partially done (2026-06-24):** Finish QSS token mechanism and migrate
  ...
  reference §5 design tokens as `@{group.token}` placeholders, and
  `ThemeManager::resolveTokens()` substitutes them from the canonical
  `tokenTable()` in `theme_manager.cpp` on every sheet load (global apply +
  ...
  single-use shadow; see `ui_design_rules.md` §3.7/§5.
...
- **Open (2026-06-24):** Migrate hard-coded local dependency paths in qmake
  ...
  follow `docs/build_and_verification.md`: root app builds under root `build\`,
