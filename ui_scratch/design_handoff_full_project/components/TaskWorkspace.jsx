...
function TWStatusLamp({ label, ok }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
      ...
      <span style={{ fontSize:8, fontWeight:700, color:'#5a5a5a', letterSpacing:'0.08em', textTransform:'uppercase' }}>{label}</span>
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: active ? `${meta.color}15` : hover ? '#2a2d2e' : 'transparent',
        border:`1px solid ${active ? meta.color+'44' : 'transparent'}`,
        transition:'all 0.13s',
      }}
    >
      <div style={{ position:'relative', flexShrink:0 }}>
        <Icon size={15} color={active ? meta.color : '#5a5a5a'}/>
        <div style={{ position:'absolute', bottom:-1, right:-1, width:5, height:5, borderRadius:'50%', background:device.connected?'#22d17a':'#e84040', border:'1px solid #1e1e1e' }}/>
      </div>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontSize:11, fontFamily:'JetBrains Mono, monospace', fontWeight:active?700:400, color:active?'#cccccc':'#9a9a9a', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{device.name}</div>
        <div style={{ fontSize:9, color:meta.color, fontWeight:700, letterSpacing:'0.05em' }}>{meta.label.toUpperCase()}</div>
      </div>
      {(hover||active) && (
        <div style={{ display:'flex', gap:2, flexShrink:0 }}>
          <button onClick={e=>{e.stopPropagation(); onMove(device);}} title="Move to task" style={{ background:'transparent', border:'none', cursor:'pointer', color:'#5a5a5a', padding:'2px', borderRadius:3, display:'flex' }}
            onMouseEnter={e=>{e.currentTarget.style.color='#2b8ce8'; e.currentTarget.style.background='#3c3c3c';}}
            onMouseLeave={e=>{e.currentTarget.style.color='#5a5a5a'; e.currentTarget.style.background='transparent';}}
          >
...
function TWFixedTab({ label, Icon, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      width:'100%', color:active?'#2b8ce8':'#5a5a5a', transition:'all 0.13s', fontFamily:'Space Grotesk, sans-serif',
    }}
      onMouseEnter={e=>{ if(!active){ e.currentTarget.style.background='#2a2d2e'; e.currentTarget.style.color='#9a9a9a'; }}}
      onMouseLeave={e=>{ if(!active){ e.currentTarget.style.background='transparent'; e.currentTarget.style.color='#5a5a5a'; }}}
    >
...
function TaskWorkspace({ task, allTasks, activeDeviceId, setActiveDeviceId, onAddDevice, onMoveDevice, onDeleteDevice, onToggleDeviceConnect }) {
  ...
  const renderContent = () => {
    ...
    if (activeDeviceId==='__patterns') return <PatternManagerPanel/>;
    if (activeDevice) {
  ...
  const breadcrumb = activeDeviceId==='__settings' ? 'Settings' : activeDeviceId==='__patterns' ? 'Pattern Manager' : activeDevice ? activeDevice.name : 'Dashboard';
  ...
  return (
    <div style={{ flex:1, display:'flex', overflow:'hidden', minWidth:0 }}>
      {/* Left device nav */}
      <div style={{ width:188, background:'#1e1e1e', borderRight:'1px solid #2a2d2e', display:'flex', flexDirection:'column', flexShrink:0 }}>
        {/* Task header */}
        <div style={{ padding:'10px 12px 8px', borderBottom:'1px solid #2a2d2e' }}>
          <div style={{ fontSize:9, color:'#454545', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:3 }}>Active Task</div>
          <div style={{ fontSize:11, fontFamily:'JetBrains Mono, monospace', fontWeight:700, color:'#cccccc', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{task.name}</div>
          <span style={{ marginTop:4, display:'inline-block', fontSize:9, fontWeight:700, color:TW_TASK_COLORS[task.type]||'#9a9a9a', background:`${TW_TASK_COLORS[task.type]||'#9a9a9a'}15`, border:`1px solid ${TW_TASK_COLORS[task.type]||'#9a9a9a'}33`, borderRadius:3, padding:'1px 6px', letterSpacing:'0.05em' }}>
            {TW_TASK_LABELS[task.type]||task.type}
          </span>
        </div>

        {/* Status lamps */}
        <div style={{ padding:'8px 6px', borderBottom:'1px solid #2a2d2e', display:'flex', justifyContent:'space-around' }}>
        ...
        {/* Pattern Manager tab — only for Localization tasks */}
        {task.type === 'LocalizationTask' && (
          <div style={{ padding:'0 8px 2px' }}>
            <TWFixedTab label="Patterns" Icon={IcoPattern} active={activeDeviceId==='__patterns'} onClick={()=>setActiveDeviceId('__patterns')}/>
          </div>
        )}

        {/* Devices section */}
        <div style={{ padding:'6px 10px 3px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <span style={{ fontSize:9, fontWeight:700, color:'#3c3c3c', textTransform:'uppercase', letterSpacing:'0.1em' }}>Devices</span>
          <button onClick={()=>onAddDevice(task.id)} style={{ background:'transparent', border:'none', cursor:'pointer', color:'#454545', padding:'2px', borderRadius:3, display:'flex' }}
            onMouseEnter={e=>{e.currentTarget.style.color='#2b8ce8'; e.currentTarget.style.background='#3c3c3c';}}
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'0 8px' }}>
          {task.devices.length===0 ? (
            <div onClick={()=>onAddDevice(task.id)} style={{ padding:'10px 8px', color:'#3c3c3c', fontSize:11, textAlign:'center', cursor:'pointer', borderRadius:5, border:'1px dashed #2a2d2e', marginTop:4 }}
              onMouseEnter={e=>e.currentTarget.style.borderColor='#2b8ce8'}
              onMouseLeave={e=>e.currentTarget.style.borderColor='#2a2d2e'}
            ><IcoPlus size={14} color="#3c3c3c"/><br/>Add device</div>
        ...
        {/* Settings tab */}
        <div style={{ padding:'2px 8px 8px', borderTop:'1px solid #2a2d2e' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        {/* Breadcrumb bar */}
        <div style={{ height:36, background:'#252526', borderBottom:'1px solid #2a2d2e', display:'flex', alignItems:'center', padding:'0 16px', gap:8, flexShrink:0 }}>
          <span style={{ fontSize:10, color:'#454545', fontFamily:'JetBrains Mono, monospace' }}>{task.name}</span>
          <span style={{ color:'#2a2d2e', fontSize:14 }}>›</span>
          <span style={{ fontSize:11, fontFamily:'JetBrains Mono, monospace', fontWeight:600, color: devMeta?.color||'#9cdcfe' }}>{breadcrumb}</span>
          ...
          <div style={{ display:'flex', gap:4 }}>
            {[IcoSave,IcoRefresh].map((Ico,i)=>(
              <button key={i} style={{ background:'transparent', border:'none', cursor:'pointer', color:'#454545', padding:'4px', borderRadius:4, display:'flex' }}
                onMouseEnter={e=>e.currentTarget.style.color='#cccccc'} onMouseLeave={e=>e.currentTarget.style.color='#454545'}
              ><Ico size={13}/></button>
            ))}
          </div>
        </div>
