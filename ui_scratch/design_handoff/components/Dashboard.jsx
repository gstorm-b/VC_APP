...
function StatCard({ label, value, unit, color = '#2b8ce8', sub }) {
  return (
    <div style={{
      background: '#0e1520', border: '1px solid #1a2540', borderRadius: 8,
      padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 4,
    }}>
      <div style={{ fontSize: 10, color: '#6b7ea0', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
        <span style={{ fontSize: 28, fontWeight: 700, color, fontFamily: 'JetBrains Mono, monospace', lineHeight: 1.1 }}>{value}</span>
        {unit && <span style={{ fontSize: 12, color: '#6b7ea0' }}>{unit}</span>}
      </div>
      {sub && <div style={{ fontSize: 11, color: '#3a4f6a' }}>{sub}</div>}
    </div>
  );
}

const LOG_COLORS = { INFO: '#5ba3f0', WARN: '#f5a623', ERROR: '#e84040', OK: '#22d17a' };

function LogRow({ time, level, message }) {
  return (
    <div style={{
      display: 'flex', gap: 12, padding: '5px 0', borderBottom: '1px solid #111820',
      fontFamily: 'JetBrains Mono, monospace', fontSize: 11, alignItems: 'flex-start',
    }}>
      <span style={{ color: '#3a4f6a', flexShrink: 0 }}>{time}</span>
      <span style={{ color: LOG_COLORS[level] || '#6b7ea0', flexShrink: 0, width: 36 }}>{level}</span>
      <span style={{ color: '#8a9ab5', lineHeight: 1.5 }}>{message}</span>
    </div>
  );
}
...
function DeviceStatusRow({ name, status, detail, onToggle }) {
  ...
  return (
    <div style={{
      ...
      padding: '10px 14px', background: '#0d1420', borderRadius: 6,
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#dce8f5' }}>{name}</div>
        <div style={{ fontSize: 10, color: '#3a4f6a', fontFamily: 'JetBrains Mono, monospace', marginTop: 1 }}>{detail}</div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color }}>{label}</span>
        <button onClick={onToggle} style={{
          background: '#1a2540', border: '1px solid #243044', borderRadius: 4,
          padding: '4px 10px', fontSize: 11, color: '#6b7ea0', cursor: 'pointer',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%', overflowY: 'auto', padding: '20px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        ...
        <StatCard label="Last Score" value="0.967" color="#dce8f5" sub="Pattern #1" />
      </div>

      {/* Charts row */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{ background: '#0e1520', border: '1px solid #1a2540', borderRadius: 8, padding: '14px 18px' }}>
          <div style={{ fontSize: 11, color: '#6b7ea0', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
            Runs / min
          </div>
          <RunChart data={chartData} />
          <div style={{ fontSize: 10, color: '#3a4f6a', marginTop: 6, fontFamily: 'JetBrains Mono, monospace' }}>
      ...
      {/* System log */}
      <div style={{ background: '#0e1520', border: '1px solid #1a2540', borderRadius: 8, padding: '14px 18px', flex: 1 }}>
        <div style={{ fontSize: 11, color: '#6b7ea0', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
          <span>System Log</span>
          <button style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#6b7ea0', display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
