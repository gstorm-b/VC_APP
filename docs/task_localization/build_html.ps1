...
foreach ($file in $files) {
    ...
    Set-Content -LiteralPath (Join-Path $DocsDir $htmlName) -Value $page -Encoding UTF8
...
Set-Content `
    -LiteralPath (Join-Path $DocsDir "task_localization_developer_docs.html") `
    -Value $combinedPage `
    -Encoding UTF8

Write-Host "Generated HTML docs in $DocsDir"
