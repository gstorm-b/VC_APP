...
function PropRow({ prop, onChange, onFocus }) {
  ...
  const inputBase = {
    background: focused ? '#252526' : '#1e1e1e',
    border: `1px solid ${changed ? '#f5a62355' : focused ? '#2b8ce8' : '#3c3c3c'}`,
    borderRadius: 3, padding: '3px 7px', color: changed ? '#f5a623' : '#cccccc',
  ...
  const renderInput = () => {
    if (prop.type === 'readonly') return (
      <span style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: '#7a7a7a', padding: '3px 7px' }}>{prop.value}</span>
    );
    if (prop.type === 'bool') return (
      <div
        ...
        style={{
          ...
          background: prop.value ? '#2b8ce8' : '#3c3c3c',
          position: 'relative', transition: 'background 0.18s', flexShrink: 0,
          border: `1px solid ${prop.value ? '#2b8ce8' : '#454545'}`,
        }}
      >
        <div style={{
    ...
    if (prop.type === 'number') return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        />
        {prop.unit && <span style={{ fontSize: 9, color: '#5a5a5a', flexShrink: 0, fontFamily: 'JetBrains Mono, monospace' }}>{prop.unit}</span>}
      </div>
    );
    // string
    return (
...
function PropGroup({ group, props, onPropChange, onFocus }) {
  ...
  return (
    <div style={{ marginBottom: 2 }}>
      ...
      >
        <IcoChevron size={10} color="#5a5a5a" open={open}/>
        <span style={{ fontSize: 10, fontWeight: 700, color: '#7a7a7a', textTransform: 'uppercase', letterSpacing: '0.1em', flex: 1 }}>
          {group.group}
        </span>
        <span style={{ fontSize: 9, color: '#454545', fontFamily: 'JetBrains Mono, monospace' }}>
          {props.length}
        </span>
      </div>
      {open && (
...
function PropertyPanel({ deviceType, taskType, context }) {
  ...
  if (collapsed) return (
    <div style={{
      width: 28, background: '#1e1e1e', borderLeft: '1px solid #3c3c3c',
      ...
      <button onClick={() => setCollapsed(false)} title="Open Properties" style={{
        background: 'transparent', border: 'none', cursor: 'pointer', color: '#5a5a5a',
      ...
      {changedCount > 0 && (
        <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#f5a623', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#1e1e1e' }}>
          {changedCount}
        </div>
      )}
    </div>
  ...
  return (
    <div style={{
      width: 268, background: '#1e1e1e', borderLeft: '1px solid #3c3c3c',
      display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden',
    }}>
      {/* Header */}
      <div style={{ padding: '8px 10px', borderBottom: '1px solid #3c3c3c', background: '#252526', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#5a5a5a" strokeWidth="2" strokeLinecap="round">
          <line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/>
        </svg>
        <span style={{ fontSize: 10, fontWeight: 700, color: '#5a5a5a', textTransform: 'uppercase', letterSpacing: '0.1em', flex: 1 }}>Properties</span>
        {changedCount > 0 && (
        ...
        <button onClick={() => setCollapsed(true)} title="Collapse" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#454545', padding: '2px', borderRadius: 3, display: 'flex' }}
      ...
      {/* Context label */}
      {context && (
        <div style={{ padding: '5px 12px', background: '#1e1e1e', borderBottom: '1px solid #2d2d2d', display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{ width: 5, height: 5, borderRadius: '50%', background: context.color || '#2b8ce8', flexShrink: 0 }}/>
          <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#7a7a7a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {context.name}
          </span>
          <span style={{ fontSize: 9, color: '#454545', marginLeft: 'auto', fontWeight: 700, letterSpacing: '0.06em' }}>
            {context.typeLabel}
          </span>
        </div>
      )}
      ...
      {/* Search */}
      <div style={{ padding: '6px 8px', borderBottom: '1px solid #2d2d2d', flexShrink: 0 }}>
        <div style={{ position: 'relative' }}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#5a5a5a" strokeWidth="2.5" strokeLinecap="round" style={{ position: 'absolute', left: 7, top: '50%', transform: 'translateY(-50%)' }}>
          ...
          <input
            ...
            style={{
              width: '100%', background: '#1e1e1e', border: '1px solid #2d2d2d',
              borderRadius: 4, padding: '5px 8px 5px 24px', color: '#9a9a9a',
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', cursor: 'pointer', color: '#5a5a5a', padding: 0, display: 'flex' }}>
              <IcoX size={10}/>
            </button>
          )}
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {resolvedSchema.length === 0 ? (
          <div style={{ padding: '20px', textAlign: 'center', color: '#454545', fontSize: 11 }}>No properties match</div>
      ...
      {/* Description footer */}
      <div style={{ padding: '8px 12px', borderTop: '1px solid #2d2d2d', background: '#1e1e1e', minHeight: 48, flexShrink: 0 }}>
        {focusedProp ? (
          <>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#2b8ce8', marginBottom: 3, fontFamily: 'JetBrains Mono, monospace' }}>{focusedProp.key}</div>
            <div style={{ fontSize: 10, color: '#7a7a7a', lineHeight: 1.5 }}>{focusedProp.desc}</div>
          </>
        ) : (
          <div style={{ fontSize: 10, color: '#454545', lineHeight: 1.5 }}>Click a property to see its description</div>
        )}
      </div>

      {/* Apply / Reset */}
      {changedCount > 0 && (
        <div style={{ padding: '8px 10px', borderTop: '1px solid #3c3c3c', display: 'flex', gap: 6, flexShrink: 0 }}>
          <button
            onClick={() => setValues({})}
            style={{ flex: 1, background: 'transparent', border: '1px solid #454545', borderRadius: 4, padding: '6px', fontSize: 11, color: '#7a7a7a', cursor: 'pointer' }}
