...
/* ─────────── shared ─────────── */
const fieldLabel = { fontSize: 11, color: '#6b7ea0', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4, display: 'block' };
const fieldInput = {
  width: '100%', background: '#0d1420', border: '1px solid #1f2d42',
  borderRadius: 5, padding: '7px 10px', color: '#dce8f5', fontSize: 12,
  fontFamily: 'JetBrains Mono, monospace', outline: 'none', boxSizing: 'border-box',
};
const sectionTitle = { fontSize: 10, color: '#3a4f6a', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 10 };
const panelCard = { background: '#0e1520', border: '1px solid #1a2540', borderRadius: 8, padding: '14px 18px' };

function PanelRow({ label, children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '6px 0', borderBottom: '1px solid #111820' }}>
      <div style={{ width: 180, fontSize: 12, color: '#6b7ea0', flexShrink: 0 }}>{label}</div>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <input
        style={{
          ...
          borderColor: focused ? '#2b8ce8' : '#1f2d42',
      ...
      />
      {unit && <span style={{ fontSize: 11, color: '#3a4f6a', flexShrink: 0 }}>{unit}</span>}
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: value ? '#2b8ce8' : '#1a2540', position: 'relative',
...
function PatternCard({ pattern, selected, onSelect }) {
  return (
    <div
      ...
      style={{
        border: `1px solid ${selected ? '#2b8ce8' : '#1f2d42'}`,
        background: selected ? '#0d1e35' : '#0d1420',
    ...
    >
      ...
      <div style={{
        width: '100%', aspectRatio: '4/3', background: '#0a1018',
        ...
        <svg width="100%" height="100%" style={{ position: 'absolute', inset: 0 }}>
          {[1,2,3,4].map(i => <line key={`h${i}`} x1="0" y1={`${i*20}%`} x2="100%" y2={`${i*20}%`} stroke="#1a2540" strokeWidth="0.5" />)}
          {[1,2,3,4].map(i => <line key={`v${i}`} x1={`${i*20}%`} y1="0" x2={`${i*20}%`} y2="100%" stroke="#1a2540" strokeWidth="0.5" />)}
        </svg>
        {/* reticle */}
        ...
        {!pattern.active && (
          <div style={{
            ...
            <span style={{ fontSize: 9, color: '#6b7ea0', fontWeight: 600, letterSpacing: '0.08em' }}>DISABLED</span>
          </div>
        )}
      </div>
      <div style={{ fontSize: 11, color: '#dce8f5', fontWeight: 600, fontFamily: 'JetBrains Mono, monospace', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pattern.name}</div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#3a4f6a' }}>{pattern.w}×{pattern.h}px</span>
    ...
    <div style={{ display: 'flex', height: '100%', gap: 0, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px 0 20px 24px', gap: 12, minWidth: 0 }}>
        ...
        <div style={{ ...panelCard, flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['LIVE','CAPTURE','FREEZE'].map(btn => (
                <button key={btn} style={{
                  background: btn === 'LIVE' ? '#2b8ce8' : '#1a2540',
                  border: 'none', borderRadius: 4, padding: '3px 10px', fontSize: 10,
                  color: btn === 'LIVE' ? '#fff' : '#6b7ea0', cursor: 'pointer', fontWeight: 700, letterSpacing: '0.06em',
                }}>{btn}</button>
              ))}
            </div>
          </div>
          {/* Camera placeholder */}
          <div style={{
            flex: 1, background: '#060a10', borderRadius: 6, position: 'relative',
            border: '1px solid #1a2540', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            <svg width="100%" height="100%" style={{ position: 'absolute', inset: 0 }}>
              {Array.from({length: 20}, (_,i) => <line key={`h${i}`} x1="0" y1={`${i*5}%`} x2="100%" y2={`${i*5}%`} stroke="#0d1520" strokeWidth="0.5"/>)}
              {Array.from({length: 30}, (_,i) => <line key={`v${i}`} x1={`${i*3.33}%`} y1="0" x2={`${i*3.33}%`} y2="100%" stroke="#0d1520" strokeWidth="0.5"/>)}
              {/* corner brackets */}
            ...
            <div style={{ position: 'absolute', bottom: 10, left: 14, fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: '#3a4f6a' }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 10, alignItems: 'center' }}>
            {[['Match Score','0.967','#22d17a'],['Position X','823 px','#dce8f5'],['Position Y','512 px','#dce8f5'],['Angle','12.4°','#dce8f5'],['Scale','1.002','#dce8f5']].map(([k,v,c]) => (
              <div key={k} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontSize: 9, color: '#3a4f6a', textTransform: 'uppercase', letterSpacing: '0.07em', fontWeight: 600 }}>{k}</span>
      ...
      <div style={{ width: 280, display: 'flex', flexDirection: 'column', padding: '20px 24px 20px 12px', gap: 12, overflow: 'hidden' }}>
        ...
        <div style={{ ...panelCard, flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div style={{ ...sectionTitle, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button style={{ background: '#1a2540', border: '1px solid #243044', borderRadius: 4, padding: '3px 8px', fontSize: 10, color: '#2b8ce8', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
...
function RegTable({ regs, type }) {
  ...
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'JetBrains Mono, monospace', fontSize: 11 }}>
      <thead>
        <tr style={{ borderBottom: '1px solid #1f2d42' }}>
          {['Address', 'Description', 'Value', 'Action'].map(h => (
            <th key={h} style={{ padding: '6px 10px', textAlign: 'left', fontSize: 10, color: '#3a4f6a', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {rows.map((row, i) => {
          ...
          return (
            <tr key={row.addr} style={{ borderBottom: '1px solid #0d1420', background: i % 2 === 0 ? 'transparent' : '#0d1420' }}>
              <td style={{ padding: '7px 10px', color: '#5ba3f0', fontWeight: 600 }}>{row.addr}</td>
              <td style={{ padding: '7px 10px', color: '#8a9ab5' }}>{row.desc}</td>
              <td style={{ padding: '7px 10px' }}>
                {isBit ? (
                  <span style={{
                    display: 'inline-block', padding: '2px 8px', borderRadius: 3,
                    fontSize: 10, fontWeight: 700, letterSpacing: '0.06em',
                    background: isOn ? '#22d17a22' : '#1a2540',
                    color: isOn ? '#22d17a' : '#3a4f6a',
                    border: `1px solid ${isOn ? '#22d17a44' : '#1f2d42'}`,
                  }}>{isOn ? 'ON' : 'OFF'}</span>
                ) : (
                  <span style={{ color: '#dce8f5' }}>{row.val}</span>
                )}
              </td>
              <td style={{ padding: '7px 10px' }}>
                {isBit ? (
                  <div style={{ display: 'flex', gap: 4 }}>
                    {['ON','OFF','TOGGLE'].map(cmd => (
                      <button key={cmd} onClick={() => {
                        ...
                        background: '#1a2540', border: '1px solid #243044', borderRadius: 3,
                        padding: '2px 7px', fontSize: 10, color: '#6b7ea0', cursor: 'pointer',
                      }}
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    <button style={{
                      background: '#1a2540', border: '1px solid #243044', borderRadius: 3,
                      padding: '3px 8px', fontSize: 10, color: '#6b7ea0', cursor: 'pointer',
                    }}>Write</button>
                  </div>
                )}
              </td>
...
function CameraPanel() {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', gap: 0, overflow: 'hidden' }}>
      {/* Camera list */}
      <div style={{ width: 260, borderRight: '1px solid #1a2540', padding: '20px 16px 20px 24px', display: 'flex', flexDirection: 'column', gap: 8, overflowY: 'auto' }}>
        <div style={{ ...sectionTitle, marginBottom: 12 }}>Available Cameras</div>
        {cameras.map((cam, i) => (
          <div key={i} onClick={() => setSelected(i)} style={{
            border: `1px solid ${selected === i ? '#2b8ce8' : '#1f2d42'}`,
            background: selected === i ? '#0d1e35' : '#0d1420',
            borderRadius: 7, padding: '12px 14px', cursor: 'pointer', transition: 'all 0.15s',
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: '#dce8f5', fontFamily: 'JetBrains Mono, monospace' }}>{cam.name}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {[['IP', cam.ip], ['Serial', cam.serial], ['Resolution', cam.res], ['Max FPS', `${cam.fps} fps`]].map(([k,v]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 10, color: '#3a4f6a' }}>{k}</span>
                  <span style={{ fontSize: 10, color: '#6b7ea0', fontFamily: 'JetBrains Mono, monospace' }}>{v}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <button style={{
          marginTop: 8, background: 'transparent', border: '1px dashed #1f2d42', borderRadius: 7, padding: '10px',
          color: '#3a4f6a', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
        }}
          onMouseEnter={e => e.currentTarget.style.borderColor = '#2b8ce8'}
          onMouseLeave={e => e.currentTarget.style.borderColor = '#1f2d42'}
        ><IcoPlus size={13} color="#3a4f6a" /> Scan Network</button>
      </div>
      ...
      <div style={{ flex: 1, padding: '20px 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ ...panelCard }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <PanelRow label="Exposure Time">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 11, color: '#6b7ea0' }}>Auto</span>
              </div>
            </PanelRow>
            <PanelRow label="Gain">
            ...
            <PanelRow label="Trigger Mode">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 11, color: '#6b7ea0' }}>{triggerMode ? 'Hardware trigger (Line1)' : 'Software trigger'}</span>
              </div>
            </PanelRow>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            ...
            <button style={{ background: '#1a2540', border: '1px solid #243044', borderRadius: 5, padding: '7px 14px', fontSize: 12, color: '#6b7ea0', cursor: 'pointer' }}>Reset Defaults</button>
          </div>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto', padding: '20px 24px', gap: 16, maxWidth: 720 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <PanelRow label="Communication Device">
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button style={{ background: '#1a2540', border: '1px solid #243044', borderRadius: 5, padding: '5px 10px', fontSize: 11, color: '#6b7ea0', cursor: 'pointer', flexShrink: 0 }}>···</button>
            </div>
          </PanelRow>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button style={{ background: 'transparent', border: '1px solid #243044', borderRadius: 5, padding: '9px 16px', fontSize: 13, color: '#6b7ea0', cursor: 'pointer' }}>Discard Changes</button>
      </div>
    </div>
  );
}
