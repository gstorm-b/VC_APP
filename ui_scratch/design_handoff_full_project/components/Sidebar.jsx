...
function SidebarBtn({ icon, title, onClick }) {
  return (
    <button title={title} onClick={onClick} style={{
      ...
      color:'#9a9a9a', padding:'3px', borderRadius:3, display:'flex', alignItems:'center'
    }}
      onMouseEnter={e=>{ e.currentTarget.style.color='#cccccc'; e.currentTarget.style.background='#454545'; }}
...
function DeviceNode({ device, taskId, isActive, onNavigate, onDelete }) {
  ...
  return (
    ...
    >
      <span style={{ color: isActive ? meta.color : '#7a7a7a', display:'flex', flexShrink:0 }}>
...
function TaskNode({ project, task, activeTaskId, activeDeviceId, onSelectTask, onNavigate, onAddDevice, onDeleteDevice, onAddTask, onDeleteTask }) {
  ...
  const color = TASK_TYPE_COLORS[task.type] || '#9a9a9a';
  ...
  return (
    <div>
      <div
        ...
        onMouseEnter={e=>{ if(!(isActive&&!activeDeviceId)) e.currentTarget.style.background='#2a2d2e'; }}
      ...
      >
        <span onClick={e=>{ e.stopPropagation(); setOpen(o=>!o); }} style={{ display:'flex', flexShrink:0 }}>
          <IcoChevron size={11} color="#7a7a7a" open={open}/>
        </span>
        <IcoTask size={13} color={isActive && !activeDeviceId ? '#2b8ce8' : '#9a9a9a'}/>
        <span style={{
          flex:1, fontSize:12, fontFamily:'JetBrains Mono, monospace',
          fontWeight: isActive && !activeDeviceId ? 600 : 400,
          color: isActive && !activeDeviceId ? '#cccccc' : '#bfbfbf',
          overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap',
        }}>{task.name}</span>
        <span style={{
        ...
        <button
          ...
          title="Add device"
          style={{ background:'transparent', border:'none', cursor:'pointer', color:'#7a7a7a', padding:'2px', borderRadius:3, display:'flex', alignItems:'center', flexShrink:0 }}
          onMouseEnter={e=>{ e.currentTarget.style.color='#2b8ce8'; e.currentTarget.style.background='#454545'; }}
          onMouseLeave={e=>{ e.currentTarget.style.color='#7a7a7a'; e.currentTarget.style.background='transparent'; }}
        ...
        <div style={{ paddingLeft:18 }}>
          {task.devices.length === 0 ? (
            <div
              ...
              style={{
                padding:'5px 10px', fontSize:10, color:'#5a5a5a', cursor:'pointer',
...
function ProjectNode({ project, activeTaskId, activeDeviceId, onSelectTask, onNavigate, onAddDevice, onDeleteDevice, onAddTask, onDeleteTask }) {
  ...
  return (
    <div style={{ marginBottom:2 }}>
      ...
      >
        <IcoChevron size={12} color="#9a9a9a" open={open}/>
        <IcoFolder size={14} color="#2b8ce8"/>
        <span style={{ flex:1, fontSize:12, fontWeight:600, color:'#cccccc', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
          {project.name}
        </span>
      ...
      {open && (
        <div style={{ paddingLeft:8 }}>
          {project.tasks.length === 0 ? (
            <div style={{ padding:'4px 12px', color:'#5a5a5a', fontSize:11, fontStyle:'italic' }}>No tasks</div>
...
function Sidebar({ projects, activeTaskId, activeDeviceId, onSelectTask, onNavigate, onAddProject, onAddTask, onDeleteTask, onAddDevice, onDeleteDevice, onOpen, onSave }) {
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ padding:'10px 12px 8px', borderBottom:'1px solid #3c3c3c', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span style={{ fontSize:10, fontWeight:700, letterSpacing:'0.1em', color:'#7a7a7a', textTransform:'uppercase' }}>Explorer</span>
      ...
      {/* Hint bar */}
      <div style={{ padding:'5px 12px', borderBottom:'1px solid #3c3c3c', background:'#1e1e1e' }}>
        <span style={{ fontSize:9, color:'#5a5a5a', letterSpacing:'0.04em' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'6px 4px' }}>
        {projects.length === 0 ? (
          <div style={{ padding:'20px 16px', color:'#7a7a7a', fontSize:12, textAlign:'center', lineHeight:1.6 }}>
            No projects.<br/>
      ...
      {/* Footer */}
      <div style={{ padding:'8px 12px', borderTop:'1px solid #3c3c3c', fontSize:10, color:'#7a7a7a', fontFamily:'JetBrains Mono, monospace', display:'flex', justifyContent:'space-between' }}>
