...
function TaskQuickCard({ task, onClick }) {
  const color = APP_TASK_COLORS[task.type]||'#6b7ea0';
  return (
    <div onClick={onClick} style={{ background:'#0e1520', border:'1px solid #1a2540', borderRadius:8, padding:'16px 18px', cursor:'pointer', transition:'all 0.15s' }}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=color; e.currentTarget.style.background='#111f30';}}
      onMouseLeave={e=>{e.currentTarget.style.borderColor='#1a2540'; e.currentTarget.style.background='#0e1520';}}
    >
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
        <span style={{ fontSize:13, fontWeight:700, color:'#dce8f5', fontFamily:'JetBrains Mono, monospace' }}>{task.name}</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:12 }}>
        {[['Devices',task.devices.length],['Connected',task.devices.filter(d=>d.connected).length],['Cameras',task.devices.filter(d=>d.type==='camera').length]].map(([k,v])=>(
          <div key={k} style={{ background:'#0a1018', borderRadius:5, padding:'6px 10px' }}>
            <div style={{ fontSize:9, color:'#3a4f6a', textTransform:'uppercase', letterSpacing:'0.07em', fontWeight:600, marginBottom:2 }}>{k}</div>
            <div style={{ fontSize:16, fontWeight:700, color:'#8a9ab5', fontFamily:'JetBrains Mono, monospace' }}>{v}</div>
          </div>
        ))}
      </div>
        ...
        {task.devices.map(dev=>{
          ...
          return (
            <div key={dev.id} style={{ display:'flex', alignItems:'center', gap:4, background:`${m.color}10`, border:`1px solid ${m.color}33`, borderRadius:4, padding:'2px 7px' }}>
              ...
              <span style={{ fontSize:10, fontFamily:'JetBrains Mono, monospace', color:'#6b7ea0' }}>{dev.name}</span>
            </div>
          );
        })}
      </div>
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'32px 40px', gap:24, overflowY:'auto' }}>
      <div>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:6 }}>
          ...
          <h1 style={{ margin:0, fontSize:22, fontWeight:800, color:'#dce8f5', letterSpacing:'-0.01em' }}>{project.name}</h1>
        </div>
        {project.description && <p style={{ margin:'0 0 0 34px', fontSize:13, color:'#6b7ea0', lineHeight:1.6 }}>{project.description}</p>}
      </div>
      <div style={{ display:'flex', gap:10 }}>
        {[['Tasks',project.tasks.length,'#2b8ce8'],['Devices',project.tasks.reduce((s,t)=>s+t.devices.length,0),'#22d17a'],['Connected',project.tasks.reduce((s,t)=>s+t.devices.filter(d=>d.connected).length,0),'#f5a623']].map(([k,v,c])=>(
          <div key={k} style={{ background:'#0e1520', border:'1px solid #1a2540', borderRadius:7, padding:'10px 20px', display:'flex', gap:10, alignItems:'center' }}>
            <span style={{ fontSize:22, fontWeight:800, color:c, fontFamily:'JetBrains Mono, monospace' }}>{v}</span>
            <span style={{ fontSize:12, color:'#6b7ea0' }}>{k}</span>
          </div>
        ))}
      </div>
      <div>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          <span style={{ fontSize:11, fontWeight:700, color:'#6b7ea0', textTransform:'uppercase', letterSpacing:'0.1em' }}>Tasks</span>
        ...
        {project.tasks.length===0 ? (
          <div style={{ background:'#0e1520', border:'1px dashed #1a2540', borderRadius:10, padding:'40px', textAlign:'center', color:'#3a4f6a' }}>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
      <svg width="56" height="56" viewBox="0 0 24 24" fill="none">
        <rect x="3" y="3" width="8" height="8" rx="1.5" fill="#1a2540"/>
        <rect x="13" y="3" width="8" height="8" rx="1.5" fill="#1a2540"/>
        <rect x="3" y="13" width="8" height="8" rx="1.5" fill="#1a2540"/>
        <circle cx="17" cy="17" r="4" fill="none" stroke="#1f2d42" strokeWidth="1.5"/>
        <circle cx="17" cy="17" r="1.5" fill="#1f2d42"/>
      </svg>
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:18, fontWeight:700, color:'#2a3a52', marginBottom:6 }}>NCR Vision Config</div>
        <div style={{ fontSize:12, color:'#1a2540', marginBottom:24 }}>Industrial computer vision configuration platform</div>
        <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
          ...
          <button onClick={onOpenProject} style={{ background:'transparent', border:'1px solid #1f2d42', borderRadius:7, padding:'11px 24px', fontSize:13, fontWeight:600, color:'#6b7ea0', cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
...
function App() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#0b0f17', fontFamily:'Space Grotesk, sans-serif', color:'#dce8f5', overflow:'hidden' }}>

      {/* Top bar */}
      <div style={{ height:44, background:'#060d18', borderBottom:'1px solid #0f1d2e', display:'flex', alignItems:'center', padding:'0 16px', gap:0, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, paddingRight:20, borderRight:'1px solid #0f1d2e', marginRight:12 }}>
          ...
          <span style={{ fontSize:13, fontWeight:800, color:'#dce8f5', letterSpacing:'-0.01em' }}>NCR<span style={{ color:'#2b8ce8' }}>vision</span></span>
        </div>
        {['File','View','Access Level','Help'].map(item=>(
          <button key={item} style={{ background:'transparent', border:'none', cursor:'pointer', padding:'0 12px', height:'100%', fontSize:12, color:'#3a5070', fontFamily:'Space Grotesk, sans-serif' }}
            onMouseEnter={e=>e.currentTarget.style.color='#dce8f5'} onMouseLeave={e=>e.currentTarget.style.color='#3a5070'}
          >{item}</button>
        ))}
        <div style={{ flex:1 }}/>
        ...
        <div style={{ background:'#0f1d2e', border:'1px solid #1a2a3a', borderRadius:5, padding:'4px 12px', fontSize:11, color:'#3a5070', display:'flex', gap:6 }}>
