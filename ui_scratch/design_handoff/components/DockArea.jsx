...
function TaskTabBar({ openTasks, activeTaskId, onSelect, onClose, onAddTab }) {
  return (
    <div style={{
      height:36, background:'#060d18', borderBottom:'1px solid #0f1d2e',
      ...
      <div style={{ display:'flex', flex:1, overflowX:'auto', overflowY:'hidden' }}>
        {openTasks.map(task => {
          ...
          const color  = DOCK_TASK_COLORS[task.type] || '#6b7ea0';
          ...
          return (
            <div
              ...
              style={{
                ...
                background: active ? '#0b1524' : 'transparent',
                borderTop: active ? `2px solid ${color}` : '2px solid transparent',
                borderBottom: active ? '1px solid #0b1524' : '1px solid transparent',
                position:'relative', transition:'background 0.12s',
                userSelect:'none', minWidth:140, maxWidth:220,
              }}
              onMouseEnter={e=>{ if(!active) e.currentTarget.style.background='#090f1c'; }}
            ...
            >
              ...
              <span style={{
                ...
                color:active?'#dce8f5':'#4a6080',
                overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1,
              }}>{task.name}</span>
              {/* device count */}
              <span style={{ fontSize:9, color:'#1f2d42', fontFamily:'JetBrains Mono, monospace', flexShrink:0 }}>
        ...
        <button onClick={onAddTab} style={{
          ...
          color:'#1f2d42', display:'flex', alignItems:'center', flexShrink:0,
          borderRight:'1px solid #0f1d2e',
        }}
          onMouseEnter={e=>{ e.currentTarget.style.color='#2b8ce8'; e.currentTarget.style.background='#090f1c'; }}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:2, padding:'0 8px', borderLeft:'1px solid #0f1d2e', flexShrink:0 }}>
        {[
          { Icon: IcoCamera, title:'Float Camera Feed', id:'camera-feed' },
          { Icon: IcoPattern, title:'Float Pattern Library', id:'pattern-lib' },
        ].map(({Icon,title,id})=>(
          <button key={id} title={title} style={{ background:'transparent', border:'none', cursor:'pointer', color:'#1f2d42', padding:'4px 6px', borderRadius:4, display:'flex', alignItems:'center', gap:5 }}
            onMouseEnter={e=>{ e.currentTarget.style.color='#6b7ea0'; e.currentTarget.style.background='#0f1d2e'; }}
            onMouseLeave={e=>{ e.currentTarget.style.color='#1f2d42'; e.currentTarget.style.background='transparent'; }}
          ><Icon size={13}/></button>
        ))}
      </div>
...
const LOG_C = { OK:'#22d17a', INFO:'#5ba3f0', WARN:'#f5a623', ERROR:'#e84040' };

function SystemLogPanel() {
  const endRef = useRef(null);
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ display:'flex', alignItems:'center', padding:'4px 12px', gap:8, borderBottom:'1px solid #0f1d2e', flexShrink:0 }}>
        <span style={{ fontSize:9, color:'#1f2d42', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', flex:1 }}>System Log</span>
        <button style={{ background:'transparent', border:'none', cursor:'pointer', color:'#1f2d42', padding:'2px 6px', borderRadius:3, fontSize:9, fontWeight:700, display:'flex', alignItems:'center', gap:4 }}>
          <IcoRefresh size={10}/> Clear
        </button>
        {['ALL','OK','WARN','ERROR'].map(f=>(
          <button key={f} style={{ background:'transparent', border:'1px solid #0f1d2e', borderRadius:3, padding:'1px 7px', fontSize:9, color:'#2a3a52', cursor:'pointer', fontWeight:700 }}
            onMouseEnter={e=>e.currentTarget.style.borderColor='#2b8ce8'} onMouseLeave={e=>e.currentTarget.style.borderColor='#0f1d2e'}
          >{f}</button>
        ))}
      </div>
      <div style={{ flex:1, overflowY:'auto', padding:'4px 0' }}>
        {BOTTOM_LOGS.map((l,i)=>(
          <div key={i} style={{ display:'flex', gap:10, padding:'3px 12px', fontFamily:'JetBrains Mono, monospace', fontSize:11 }}>
            <span style={{ color:'#1f2d42', flexShrink:0, width:56 }}>{l.time}</span>
            <span style={{ color:LOG_C[l.level]||'#6b7ea0', flexShrink:0, width:34, fontWeight:700 }}>{l.level}</span>
...
function PLCMonitorPanel() {
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ display:'flex', alignItems:'center', padding:'4px 12px', gap:8, borderBottom:'1px solid #0f1d2e', flexShrink:0 }}>
        <span style={{ fontSize:9, color:'#1f2d42', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', flex:1 }}>PLC Register Monitor</span>
        ...
        <button onClick={()=>setConnected(c=>!c)} style={{ background:connected?'#2a1a1a':'#1a2540', border:`1px solid ${connected?'#e8404044':'#243044'}`, borderRadius:3, padding:'2px 8px', fontSize:9, color:connected?'#e84040':'#6b7ea0', cursor:'pointer', fontWeight:700 }}>
          {connected?'Stop':'Connect'}
        </button>
        <button style={{ background:'transparent', border:'none', cursor:'pointer', color:'#1f2d42', padding:'2px', display:'flex' }}><IcoPlus size={12}/></button>
      </div>
          ...
          <thead style={{ position:'sticky', top:0, background:'#060d18', zIndex:1 }}>
            <tr>
              {['Address','Name','Value','Type','Unit'].map(h=>(
                <th key={h} style={{ padding:'4px 10px', textAlign:'left', fontSize:9, color:'#1f2d42', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', borderBottom:'1px solid #0f1d2e', whiteSpace:'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {regs.map((r,i)=>(
              <tr key={r.addr} style={{ borderBottom:'1px solid #080f18', background:i%2===0?'transparent':'#060c16' }}>
                <td style={{ padding:'4px 10px', color:'#5ba3f0', fontWeight:600 }}>{r.addr}</td>
                <td style={{ padding:'4px 10px', color:'#3a5070' }}>{r.name}</td>
                <td style={{ padding:'4px 10px' }}>
                  {r.type==='bit' ? (
                    <span style={{ display:'inline-block', padding:'1px 8px', borderRadius:3, fontSize:9, fontWeight:800, letterSpacing:'0.06em', background:r.val?'#22d17a22':'#1a2540', color:r.val?'#22d17a':'#2a3a52', border:`1px solid ${r.val?'#22d17a44':'#1a2540'}` }}>
                      {r.val?'ON':'OFF'}
                    </span>
                  ) : (
                    <span style={{ color:'#dce8f5', fontWeight:600 }}>{r.val}</span>
                  )}
                </td>
                <td style={{ padding:'4px 10px', color:'#1f2d42', fontSize:9, textTransform:'uppercase' }}>{r.type}</td>
                <td style={{ padding:'4px 10px', color:'#1f2d42', fontSize:9 }}>{r.unit||'—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function BottomDock({ height, onResize, onClose }) {
  ...
  return (
    <div style={{ height, background:'#060d18', borderTop:'1px solid #0f1d2e', display:'flex', flexDirection:'column', flexShrink:0 }}>
      {/* Resize handle */}
      ...
      />
      {/* Tab bar */}
      <div style={{ height:28, display:'flex', alignItems:'stretch', borderBottom:'1px solid #0f1d2e', flexShrink:0 }}>
        {BOTTOM_PANELS.map(p=>{
          const active = activePanel===p.id;
          return (
            <button key={p.id} onClick={()=>setActivePanel(p.id)} style={{
              display:'flex', alignItems:'center', gap:6, padding:'0 14px',
              background:active?'#0b1524':'transparent', border:'none', borderTop:`2px solid ${active?'#2b8ce8':'transparent'}`,
              cursor:'pointer', color:active?'#6b7ea0':'#2a3a52', fontFamily:'Space Grotesk, sans-serif',
              borderRight:'1px solid #0f1d2e', fontSize:11, fontWeight:active?600:400, transition:'all 0.12s',
            }}
              onMouseEnter={e=>{ if(!active) e.currentTarget.style.background='#090f1c'; }}
              onMouseLeave={e=>{ if(!active) e.currentTarget.style.background='transparent'; }}
            >
        ...
        <div style={{ flex:1 }}/>
        <button onClick={onClose} style={{ background:'transparent', border:'none', cursor:'pointer', color:'#1f2d42', padding:'0 10px', display:'flex', alignItems:'center' }}
          onMouseEnter={e=>e.currentTarget.style.color='#6b7ea0'} onMouseLeave={e=>e.currentTarget.style.color='#1f2d42'}
          title="Close panel"
        ><IcoX size={12}/></button>
      </div>
      {/* Content */}
  ...
  return (
    <div
      ...
      style={{
        ...
        background:'#0b1524', border:`1px solid ${color}44`,
    ...
    >
      ...
      <div
        ...
        style={{
          height:32, background:'#060d18', borderBottom:`1px solid ${color}33`,
      ...
      >
        ...
        <span style={{ flex:1, fontSize:11, fontFamily:'JetBrains Mono, monospace', fontWeight:700, color:'#6b7ea0' }}>{title}</span>
...
function TaskPickerDropdown({ allTasks, openTaskIds, onSelect, onClose }) {
  ...
  return (
    <div style={{
      ...
      background:'#0b1524', border:'1px solid #1a2540', borderRadius:7,
      boxShadow:'0 16px 40px rgba(0,0,0,0.6)', padding:'6px', minWidth:240,
    }}>
      <div style={{ fontSize:9, color:'#1f2d42', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', padding:'4px 8px 6px' }}>Open Task</div>
      {available.length===0 ? (
        <div style={{ padding:'12px 8px', color:'#1f2d42', fontSize:11, textAlign:'center' }}>All tasks already open</div>
      ) : available.map(t=>{
        const color = DOCK_TASK_COLORS[t.type]||'#6b7ea0';
        return (
          ...
          >
            ...
            <span style={{ fontSize:12, fontFamily:'JetBrains Mono, monospace', color:'#8a9ab5' }}>{t.name}</span>
            <span style={{ marginLeft:'auto', fontSize:9, color:'#1f2d42' }}>{t.devices.length}d</span>
          </div>
        );
      })}
      <div style={{ borderTop:'1px solid #0f1d2e', marginTop:4, paddingTop:4 }}>
        <div onClick={onClose} style={{ padding:'5px 10px', fontSize:11, color:'#1f2d42', cursor:'pointer', borderRadius:4 }}
          onMouseEnter={e=>e.currentTarget.style.background='#111f30'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}
        >Cancel</div>
      </div>
    </div>
  );
...
function DockWorkspace({
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', position:'relative' }}>
      ...
      {openTasks.length === 0 ? (
        <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', color:'#1f2d42', gap:12 }}>
          <IcoTask size={40} color="#0f1d2e"/>
          <div style={{ fontSize:14, fontWeight:700, color:'#1f2d42' }}>No tasks open</div>
          <button onClick={()=>setShowPicker(true)} style={{ background:'transparent', border:'1px solid #1a2540', borderRadius:6, padding:'8px 20px', fontSize:12, color:'#2a3a52', cursor:'pointer' }}
            onMouseEnter={e=>e.currentTarget.style.borderColor='#2b8ce8'} onMouseLeave={e=>e.currentTarget.style.borderColor='#1a2540'}
          >Open a task</button>
        </div>
      ) : activeTask ? (
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          {showBottom ? (
            ...
            <div style={{ height:24, background:'#060d18', borderTop:'1px solid #0f1d2e', display:'flex', alignItems:'center', padding:'0 10px', gap:8 }}>
              <button onClick={()=>setShowBottom(true)} style={{ background:'transparent', border:'none', cursor:'pointer', color:'#1f2d42', fontSize:10, fontWeight:700, display:'flex', alignItems:'center', gap:6 }}
                onMouseEnter={e=>e.currentTarget.style.color='#2b8ce8'} onMouseLeave={e=>e.currentTarget.style.color='#1f2d42'}
              >
